...
export default function CotizarLowCostPage() {
  ...
  const baseRate = 950;
  const zoneDiff = Math.abs(oBarrio.distanceVal - dBarrio.distanceVal) * 750;
  
  const weightSurcharge = peso.includes('Mediano') ? 300 : peso.includes('Grande') ? 700 : 0;
  
  const totalPrice = Math.round(baseRate + zoneDiff + weightSurcharge);

  const getWhatsAppLink = () => {
    const text = `Hola Envios DosRuedas! Solicito un *Envío LowCost* cotizado desde la web:
📍 *Origen:* ${origen}
📍 *Destino:* ${destino}
📦 *Tamaño/Peso:* ${peso}
⏰ *Plazo:* Entrega en el día (antes de las 19:00 hs)
...
💵 *Precio Estimado:* $${totalPrice} ARS`;
  ...
  return (
    <div className="relative bg-white py-16 lg:py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="absolute bottom-10 left-10 w-80 h-80 bg-egyptian-blue/5 rounded-full blur-3xl -z-10"></div>

      <div className="max-w-7xl mx-auto relative z-10">
        
        {/* Header Title */}
        <div className="text-center space-y-4 mb-16">
          <span className="font-bebas text-md tracking-widest text-brand-blue-dark bg-sunbeam-yellow px-3.5 py-1.5 rounded-full uppercase inline-flex items-center gap-1.5 shadow-sm">
            <BadgePercent className="w-4 h-4 text-egyptian-blue" /> AHORRO • LOWCOST (ENTREGA EN EL DÍA)
          </span>
          <h1 className="font-anton text-4xl sm:text-5xl lg:text-6xl text-brand-dark tracking-tight uppercase leading-none">
            COTIZADOR LOWCOST MDQ
          </h1>
          <p className="text-slate-500 text-md sm:text-lg leading-relaxed max-w-2xl mx-auto font-sans">
            La opción más inteligente y económica para tus envíos masivos de e-commerce. Despachá antes de las 13:00 hs y entregamos en el día antes de las 19:00 hs.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl p-8 border border-slate-100 shadow-xl space-y-6">
            <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-sans">Máxima eficiencia en ruteo y costos compartidos</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="font-bebas text-md text-slate-700 tracking-wider flex items-center gap-1.5">
                    <Clock className="w-4 h-4 text-egyptian-blue" /> Plazo de Entrega Garantizado
                  </label>
                  <div className="px-4 py-3 bg-slate-100 rounded-lg text-xs font-mono text-slate-600 font-bold uppercase flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    ENTREGA EN EL DÍA (HASTA LAS 19:00 hs)
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label htmlFor="observaciones" className="font-bebas text-md text-slate-700 tracking-wider block">
                  Instrucciones o Referencias Adicionales
                </label>
                <textarea
                  id="observaciones"
                  rows={3}
                  ...
                  placeholder="Ej: Dejar en portería si no responde. Paquete de venta online listo para entregar..."
                  className="w-full px-4 py-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-egyptian-blue bg-slate-50/50"
                />
              </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-egyptian-blue text-white rounded-2xl p-8 border border-brand-blue-dark shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-1.5 pb-4 border-b border-white/10 relative z-10">
                <span className="text-xs font-mono text-sunbeam-yellow uppercase tracking-widest block font-bold">Ahorro Máximo Activado</span>
                <h3 className="font-anton text-3xl tracking-wide uppercase">SERVICIO LOWCOST</h3>
              </div>

              <div className="space-y-3 font-mono text-sm relative z-10 text-slate-200">
                <div className="flex justify-between">
                  <span>Tarifa Base LowCost:</span>
                  <span className="text-white">$950.00</span>
                </div>
                <div className="flex justify-between">
                  <span>Adicional por Cobertura:</span>
                  <span className="text-white">Reducido</span>
                </div>
                <div className="flex justify-between">
                  <span>Adicional por Peso/Tamaño:</span>
                  <span className="text-white">
                    {peso.includes('Chico') ? '$0.00' : peso.includes('Mediano') ? '$300.00' : '$700.00'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Ruta Consolidada Optim.:</span>
                  <span className="text-emerald-300 font-bold">¡INCLUIDO!</span>
                </div>
              </div>

              <div className="pt-6 border-t border-white/10 relative z-10 flex justify-between items-baseline">
                <span className="font-bebas text-2xl text-slate-200 uppercase">PRECIO ESTIMADO:</span>
                <div className="text-right">
                  <span className="font-anton text-4xl text-sunbeam-yellow font-bold">${totalPrice}</span>
                  <span className="text-[10px] font-mono block text-white/70 uppercase">ARS (IVA Incluido)</span>
                </div>
              </div>

              <div className="p-3 bg-white/5 border border-white/10 rounded-xl text-[11px] font-sans leading-relaxed text-slate-200">
                🚚 <span className="font-bold text-white">REQUISITO CLAVE:</span> Despachá tu bulto antes de las 13:00 hs para que entre en la ruta diaria consolidada y sea entregado hoy mismo antes de las 19:00 hs.
              </div>
              ...
              >
                <MessageSquare className="w-5 h-5 text-sunbeam-yellow fill-sunbeam-yellow" /> ENVIAR PEDIDO POR WHATSAPP
              </a>
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
              <h4 className="font-bebas text-md text-slate-800 tracking-wider flex items-center gap-1.5 uppercase">
                <TrendingDown className="w-4 h-4 text-egyptian-blue" /> ¿Cuándo elegir LowCost?
              </h4>
              <p className="text-xs text-slate-500 leading-relaxed font-sans">
                Es la opción favorita para tiendas de e-commerce y PyMEs locales que realizan entregas recurrentes pero no tienen una urgencia de minutos. Al agrupar envíos en rutas inteligentes optimizadas por distritos, logramos reducir el costo logístico a menos de la mitad, manteniendo la excelencia de entrega en el mismo día.
              </p>
            </div>
